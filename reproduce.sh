...
#     python experiments/highd_arch.py --run       # ~25 min (architecture sweep)
#     python experiments/wave.py --train    # ~25 min (wave equation, 2 ICs x 3 seeds)
#     python experiments/wave_leapfrog.py   # ~3 s   (its classical baseline; no training)
...
step "regenerate all 21 figures from committed artifacts" experiments/reproduce_figures.py
